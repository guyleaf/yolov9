...
class BaseModel(nn.Module):
    def forward(self, x, *args, loss=False, **kwargs):
        """
        Perform forward pass of the model for either training or inference.

        Args:
            x (torch.Tensor): Input tensor for inference.
            loss (bool): Inference in training mode (loss calculation).

        Returns:
            (Any): The output of the network.
        """
        if loss:  # for cases of training and validating while training.
    ...
    def predict(self, x, profile=False, visualize=False, augment=False):
        ...
        Returns:
            (Any): The last output of the model.
        """
        if augment:
    ...
    def info(self, verbose=False, img_size=640):
        ...
        Args:
            ...
            img_size (int): The size of the image that the model will be trained on.
        """
    ...
    def loss(self, x):
        """
        Perform a forward pass for loss calculation through the network.
...
class DetectionModel(BaseModel):
    ...
    def _predict_augment(self, x):
        """Perform augmentations on input image x and return augmented inference."""
        # TODO: support augment=True if using Dualxxx and Triplexxx head.
