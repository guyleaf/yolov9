...
DATASETS_DIR = Path(os.getenv('YOLOv5_DATASETS_DIR', WORKDIR_ROOT / 'datasets'))  # global datasets directory
...
def exclude_dict(d, exclude=()):
    return {k: v for k, v in d.items() if all(x not in k for x in exclude)}
...
def strip_optimizer(f='best.pt', s=''):  # from .general import *; strip_optimizer()
